...
OpenGC::RasterMapManager *OpenGC::RasterMapManager::m_instance = nullptr;
  
OpenGC::RasterMapManager::RasterMapManager()
  : m_Ready(false)
{
...
OpenGC::RasterMapManager *OpenGC::RasterMapManager::getInstance(void) {
...
void OpenGC::RasterMapManager::SetCachePath(CacheFormat format, const std::string& path, const std::string& mapType) {
  if (format == RMM_CACHE_MGMAPS) {
    m_CachePrefix = path;
    m_MapType = mapType;
    m_Ready = true;
    LogPrintf("OpenGC::RasterMapManager: Loading MGMaps cache of type %s from %s\n", mapType.c_str(), path.c_str());
  }
...
void OpenGC::RasterMapManager::GetTileCoordsForLatLon(unsigned int& x, unsigned int& y, float& fx, float& fy, double lat, double lon, const unsigned int zoom) {
...
OpenGC::RasterMapTile *OpenGC::RasterMapManager::GetTile(const unsigned int zoom, const unsigned int x, const unsigned int y) {	
  if (m_Ready) {
    ...
    if (snprintf(filenameBuf, sizeof(filenameBuf), "%s/%s_%d/%d_%d.mgm", m_CachePrefix.c_str(), m_MapType.c_str(), zoom, absX, absY) == sizeof(filenameBuf)) {
      LogPrintf("OpenGC::RasterMapManager: Could not work out what file the requested tile is in.\n");
      return NULL;
    }
    ...
    if (!mgmFile) {
      LogPrintf("OpenGC::RasterMapManager: Provided cache does not contain requested tile, ignoring\n");
      return NULL;
    }
	...
	if (tmp->relX == relX && tmp->relY == relY) {
	  ...
	  fseek(mgmFile, 0, SEEK_END);
	    ...
	    if (length >= READ_BUFFER_SIZE) {
	      LogPrintf("OpenGC::RasterMapManager: can't read JPEG longer than read buffer; probable malformed cache.\n");
	      return NULL;
	    }
	    ...
	    if (length != fread(m_ReadBuffer, 1, length, mgmFile)) {
	      LogPrintf("OpenGC::RasterMapManager: end of file reached before JPEG was completely read, malformed cache.\n");
	      return NULL;
	    }
...
unsigned char *OpenGC::RasterMapManager::DecodeJPEG(unsigned char *readBuffer, unsigned int length, unsigned int& width, unsigned int& height) {
  ...
  if (tinyjpeg_parse_header(jdec, readBuffer, length) < 0) {
    LogPrintf("OpenGC::RasterMapManager: JPEG codec error: %s", tinyjpeg_get_errorstring(jdec));
    return NULL;
  }
...
unsigned char *OpenGC::RasterMapManager::DecodePNG(unsigned char *readBuffer, unsigned int length, unsigned int& width, unsigned int& height) {
  ...
  if (error) {
    printf("OpenGC::RasterMapManager: PNG codec error number %d\n", error);
    return NULL;
  }
